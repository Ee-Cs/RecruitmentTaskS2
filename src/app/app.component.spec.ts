...
describe('AppComponent', () => {
   ...
   * @returns void
   */
  it(`should have the 'RecruitmentTaskS2' title`, async () => {
    // GIVEN
    // WHEN
    // THEN
    expect(component.title).toEqual('RecruitmentTaskS2');
  });
  /**
  ...
  it('should render all menu items using harness', async () => {
    ...
    expect(menuItemHarnesses.length).toBe(3);
    menuTexts = await Promise.all(menuItemHarnesses.map(item => item.getText()));
    expect(menuTexts[0]).toContain(MENU_ITEMS['ManageMissions'].label);
    expect(menuTexts[1]).toContain(MENU_ITEMS['TransferRockets'].label);
    expect(menuTexts[2]).toContain(MENU_ITEMS['CreatePDFReports'].label);
  });
});
